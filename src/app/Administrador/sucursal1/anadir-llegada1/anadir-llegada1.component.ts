...
@Component({
  selector: 'app-anadir-llegada1',
  templateUrl: './anadir-llegada1.component.html',
  styleUrls: ['./anadir-llegada1.component.css']
})
export class AnadirLlegada1Component implements OnInit {
      ...
      if (this.llegadas1.cantidad_llegada) {
        if (!this.validateCantidad(this.llegadas1.cantidad_llegada)) {
          this.sucursalService.agregarLlegadasSucursal1(this.llegadas1).subscribe(data => {
            ...
            this.router.navigate(['/llegadas-sucursal1']);
          });
        } else {
