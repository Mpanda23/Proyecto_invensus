...
@Component({
  selector: 'app-anadir-llegada2',
  templateUrl: './anadir-llegada2.component.html',
  styleUrls: ['./anadir-llegada2.component.css']
})
export class AnadirLlegada2Component implements OnInit {
      ...
      if (this.llegadas2.cantidad_llegada) {
        if (!this.validateCantidad(this.llegadas2.cantidad_llegada)) {
          this.sucursalService.agregarLlegadasSucursal2(this.llegadas2).subscribe(data => {
            ...
            this.router.navigate(['/llegadas-sucursal2']);
          });
        } else {
